...
insns_out = sys.argv[5]
mem_out = sys.argv[6]
cycles_out = sys.argv[7]
...
with open (insns_out,'w') as insns_output, open(mem_out,'w') as mem_output, open(cycles_out,'w') as cycles_output:
 with open(num_insns,'r') as f1, open(num_accesses_file,'r') as f2,open(hits_file,'r') as f3,open(trace_nos,'r') as f4:
  for line1, line2, line3, line4 in zip(f1,f2,f3,f4):
   ...
   insns_output.write(line4 + " " + str(l1+l2)+"\n")
   mem_output.write(line4 + " " + str(l2)+"\n")
   cycles_output.write(line4 + " " + str(perf)+"\n")
